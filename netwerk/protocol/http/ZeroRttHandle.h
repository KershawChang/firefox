...
class ZeroRttHandle {
 public:
  ...
  // Corresponds to nsAHttpTransaction::ReadSegments. When the caller
  // is in the 0-RTT flow, seeks the real transaction's request stream
  // to its recorded offset, forwards bytes to aReader, and advances
  // the offset.
  nsresult ReadSegments(HappyEyeballsTransaction* aCaller,
  ...
  // Corresponds to nsAHttpTransaction::WriteSegments. Always returns
  // NS_BASE_STREAM_CLOSED — racer HTs drop response bytes, and the
  // post-adopt path never reaches this function (the carrier swap in
  // HT::Adopt points the carrier at the real txn).
  nsresult WriteSegments(HappyEyeballsTransaction* aCaller,
                         nsAHttpSegmentWriter* aWriter, uint32_t aCount,
