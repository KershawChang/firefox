...
nsresult ZeroRttHandle::ReadSegments(HappyEyeballsTransaction* aCaller,
                                     nsAHttpSegmentReader* aReader,
                                     uint32_t aCount, uint32_t* aCountRead) {
  *aCountRead = 0;

  Maybe<uint64_t>& offset = aCaller->Request0RttStreamOffset();
  if (offset.isNothing()) {
    // Not in 0-RTT flow — behave speculatively.
  ...
  if (!seekable) {
    ...
    LOG(("ZeroRttHandle::ReadSegments %p caller=%p stream not seekable", this,
         aCaller));
    return NS_BASE_STREAM_CLOSED;
  }
  nsresult rv = seekable->Seek(nsISeekableStream::NS_SEEK_SET,
                               static_cast<int64_t>(offset.value()));
  if (NS_FAILED(rv)) {
    LOG(("ZeroRttHandle::ReadSegments %p caller=%p seek failed rv=%x", this,
         aCaller, static_cast<uint32_t>(rv)));
    return rv;
  }
  ...
  if (NS_SUCCEEDED(rv) && *aCountRead > 0) {
    offset = Some(offset.value() + *aCountRead);
    LOG(("ZeroRttHandle::ReadSegments %p caller=%p read=%u newOffset=%" PRIu64,
         this, aCaller, *aCountRead, offset.value()));
...
nsresult ZeroRttHandle::Finish0RTT(HappyEyeballsTransaction* aCaller,
  ...
  if (aCaller->Request0RttStreamOffset().isNothing()) {
    // Caller wasn't in the 0-RTT flow; shouldn't normally be called.
    return NS_OK;
  }
  nsHttpTransaction* realTxn = ResolveRealTxn(mHet);
  if (!realTxn) {
...
nsresult ZeroRttHandle::WriteSegments(HappyEyeballsTransaction* aCaller,
                                      nsAHttpSegmentWriter* aWriter,
                                      uint32_t aCount,
                                      uint32_t* aCountWritten) {
  *aCountWritten = 0;
  // Pre-winner, the racer HTs drop response bytes here. Post-winner,
  // HT::Adopt has already run the carrier's SwapTransaction so the
  // carrier drives the real txn directly — HT::WriteSegments (and
  // therefore this function) is no longer invoked on the winning path.
  return NS_BASE_STREAM_CLOSED;
}
